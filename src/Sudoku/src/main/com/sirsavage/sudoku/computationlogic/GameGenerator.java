...
class GameGenerator {
    ...
    //Generates a 9x9 grid and checks for the validity with the solver. If invalid, a new grid is generated until a valid one is found.
    private static int[][] getSolvedGame() {
        ...
        for (int value = 1; value <= GRID_BOUNDARY; value++) {
            //allocations is the number of times in which a square has been given the same value.
            int allocations = 0;

            //If an allocation results in an invalid game, we reset them back to 0 and restart generating process.
            int interrupt = 0;

            //Keep track of what has been allocated in the current frame of the loop
            List<Coordinates> allocTracker = new ArrayList<>();

            //As a failsafe, if we keep rolling back allocations on the most recent frame, and the game still has no solution, after 500 times we reset the entire board entirely and start again.
            int attempts = 0;
    ...
    //Takes an already solvable game and randomly picks squares to display the values for.
